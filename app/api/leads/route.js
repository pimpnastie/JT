...
async function initializeTables(sql) {
  await sql`
    CREATE TABLE IF NOT EXISTS leads (
      ...
      assigned_agent TEXT DEFAULT 'Unassigned', -- TRACKING AGENT LINK
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS deal_trackers (
      ...
      assigned_agent TEXT DEFAULT 'Jeremy Thieroff', -- DEFAULT TO FOUNDING AGENT
...
export async function POST(request) {
  try {
    ...
    if (body.action === 'create_deal') {
      ...
      return new Response(JSON.stringify({ status: 'deal created', id: result[0].id }), { status: 200 });
    }
    ...
    if (body.action === 'convert_lead') {
      ...
      const result = await sql(
        ...
        [body.lead_id, leadProfile.name, body.property_address || 'TBD Address', 'Mutual Acceptance', body.deal_side || 'Buyer', body.assigned_agent || leadProfile.assigned_agent]
      );
      return new Response(JSON.stringify({ status: 'success', id: result[0].id }), { status: 200 });
    }

    // Public Inbound Lead Generation
    await sql(
...
export async function PUT(request) {
  try {
    ...
    // Check if we are running an Agent Transfer modification request
    if (body.action === 'reassign_agent') {
      ...
      return new Response(JSON.stringify({ status: 'transfer complete' }), { status: 200 });
    }
